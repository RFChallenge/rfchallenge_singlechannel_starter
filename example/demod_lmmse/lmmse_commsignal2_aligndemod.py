...
import numpy as np
import pickle
from align_lmmse_estimator import align_lmmse_separate
...
def main():
    ...
    for idx in all_test_idx:
        ...
        sig1_mmse, sig2_mmse = align_lmmse_separate(sig_mixture, soi_type='QPSK', interference_sig_type=interference_sig_type)
        ...
        if len(all_sinr)%100 == 0:
            pickle.dump((all_ber, all_default_ber, all_sdr, all_sinr), open(os.path.join(output_folder, f'lmmse_{interference_sig_type}_{val_or_test}_demod_aligned.pickle'),'wb'))
    pickle.dump((all_ber, all_default_ber, all_sdr, all_sinr), open(os.path.join(output_folder, f'lmmse_{interference_sig_type}_{val_or_test}_demod_aligned.pickle'),'wb'))
